...
const DepartmentNode = {
  ...
  setup(props) {
    return () => h('div', { class: `${props.level === 0 ? 'tree-root' : 'tree-child'} ${props.selectedId === props.item.id ? 'tree-selected' : ''}`, onClick: (event) => { event.stopPropagation(); props.onSelect?.(props.item) } }, [
      ...
      ...(props.item.children || []).map(child => h(DepartmentNode, { key: child.id, item: child, level: props.level + 1, onSelect: props.onSelect, selectedId: props.selectedId })),
    ])
  },
}
...
function openDepartmentEditor(department) {
  ...
  departmentDraft.value = { name: department.name, parent_id: department.parent_id || '' }
  departmentModal.value = true
}
function editSelectedDepartment() { if (selectedDepartment.value) openDepartmentEditor(selectedDepartment.value) }
function removeSelectedDepartment() { if (selectedDepartment.value) deleteDepartment(selectedDepartment.value) }

function askConfirm(title, message, action) { confirmDialog.value = { title, message, action } }
function resolveConfirm(accepted) { const dialog = confirmDialog.value; confirmDialog.value = null; if (accepted) dialog?.action?.() }
...
async function createDepartment() {
  ...
  busy.value = true
  try {
    const result = await api(editingDepartment.value ? `/api/organization/departments/${editingDepartment.value.id}` : '/api/organization/departments', {
      method: editingDepartment.value ? 'PATCH' : 'POST',
      body: JSON.stringify({ name, parent_id: departmentDraft.value.parent_id || null }),
    })
    notice.value = editingDepartment.value ? `部门「${result.name}」已更新` : `部门「${result.name}」已创建`
    closeModals()
    await load()
  } catch (exception) {
    showError(exception)
...
async function deleteDepartment(department) {
  ...
  askConfirm(hasChildren ? '确认级联删除' : '确认删除部门', hasChildren ? `「${department.name}」包含 ${department.children.length} 个下级部门，将一并删除。` : `确定删除「${department.name}」吗？`, () => doDeleteDepartment(department, hasChildren))
}
async function doDeleteDepartment(department, hasChildren) {
  try {
...
async function deleteUser(user) {
  askConfirm('确认删除账号', `删除「${user.username}」后将无法恢复。`, () => doDeleteUser(user))
}
async function doDeleteUser(user) {
  try {
    await api(`/api/organization/users/${user.id}`, { method: 'DELETE' })
...
async function deleteRole(role) {
  askConfirm('确认删除角色', `删除「${role.name}」后其权限配置将被移除。`, () => doDeleteRole(role))
}
...
<template>
  <section class="organization-page"><div class="title-row"><div><div class="eyebrow">ORGANIZATION & SETTINGS</div><h1>组织架构与系统配置</h1><p>管理部门范围、角色权限和底层模型接口。</p></div><button @click="save">保存配置</button></div><p v-if="saved" class="notice">配置已保存</p><p v-if="notice" class="notice">{{notice}}</p><p v-if="error" class="error">{{error}}</p>
    ...
    <article v-if="tab==='departments'" class="card settings-panel"><div class="panel-heading"><div><b>部门树形结构</b><small>{{loading ? '正在同步组织数据…' : '部门权限默认覆盖下级，用户只能归属一个部门'}}</small></div><div class="row-actions"><button class="secondary" @click="openDepartment">+ 新增部门</button><button class="action-button" :disabled="!selectedDepartment" @click="editSelectedDepartment">编辑</button><button class="action-button danger" :disabled="!selectedDepartment" @click="removeSelectedDepartment">删除</button></div></div><div class="tree"><DepartmentNode v-for="root in departments" :key="root.id || root.code" :item="root" :level="0" :on-select="(item) => selectedDepartment = item" :selected-id="selectedDepartment?.id" /></div><div v-if="!departments.length && !loading" class="empty compact-empty">暂无部门数据</div></article>
    <article v-else-if="tab==='users'" class="card settings-panel"><div class="panel-heading"><div><b>用户账号状态</b><small>支持停用、转移部门和调整角色</small></div><button class="secondary" @click="openInvite">+ 邀请用户</button></div><div class="table-wrap"><table><thead><tr><th>用户</th><th>所属部门</th><th>角色</th><th>账号状态</th><th>操作</th></tr></thead><tbody><tr v-for="user in users" :key="user.id || user.username"><td><div class="user-cell"><span class="avatar">{{user.username.slice(0,1).toUpperCase()}}</span><div><b>{{user.username}}</b><small>{{user.id}}</small></div></div></td><td>{{user.department || '未分配'}}</td><td><span class="role-chip">{{user.role}}</span></td><td><label class="switch-label compact"><input v-model="user.active" type="checkbox" @change="toggleUser(user)"><i></i><span>{{user.active?'正常':'已停用'}}</span></label></td><td><div class="row-actions"><button class="action-button" @click="openUserEditor(user)">编辑</button><button class="action-button danger" @click="deleteUser(user)">删除</button></div></td></tr></tbody></table><div v-if="!users.length && !loading" class="empty compact-empty">暂无用户数据</div></div></article>
    <article v-else-if="tab==='roles'" class="card settings-panel"><div class="panel-heading"><div><b>角色功能操作权限树</b><small>管理员可将权限赋予角色，权限变更会立即记录审计日志</small></div><button class="secondary" @click="roleModal=true">+ 新建角色</button></div><div class="role-list"><div v-for="role in roles" :key="role.id || role.name" class="role-card"><div class="role-card-head"><div><b>{{role.name}}</b><small>{{role.code}}</small></div><div class="row-actions"><button class="action-button" @click="roleEditor=role">编辑权限</button><button class="action-button danger" @click="deleteRole(role)">删除</button></div></div><div v-if="roleEditor===role" class="permission-editor"><label v-for="permission in permissionOptions" :key="permission"><input v-model="role.permissions" type="checkbox" :value="permission">{{permission}}</label><button class="secondary" @click="saveRole(role)">保存权限</button></div><div v-else class="permission-chips"><span v-for="permission in role.permissions" :key="permission" class="permission-chip"><i>✓</i>{{permission}}</span></div></div></div></article>
