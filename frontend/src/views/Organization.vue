  ...
  setup(props) {
    return () => h('div', { class: props.level === 0 ? 'tree-root' : 'tree-child' }, [
      ...
      h('span', { class: 'tree-actions' }, [
        h('button', { class: 'action-button', onClick: (event) => { event.stopPropagation(); props.onEdit?.(props.item) } }, '编辑'),
        h('button', { class: 'action-button danger', onClick: (event) => { event.stopPropagation(); props.onDelete?.(props.item) } }, '删除'),
      ]),
      ...(props.item.children || []).map(child => h(DepartmentNode, { key: child.id, item: child, level: props.level + 1, onEdit: props.onEdit, onDelete: props.onDelete })),
    ])
  },
}
