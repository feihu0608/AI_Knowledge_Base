...
function openEdit(item){ editItem.value=item; editDraft.value={question:item.question, answer:item.answer} }
function saveEdit(){ if(!editItem.value)return; editItem.value.question=editDraft.value.question; editItem.value.answer=editDraft.value.answer; editItem.value.status='已采纳'; notice.value=`FAQ「${editItem.value.question}」已保存并发布。`; editItem.value=null; setTimeout(()=>notice.value='', 2600) }
