...
async function load() { try { bases.value = await api('/api/knowledge-bases'); selected.value ||= bases.value[0]; error.value = '' } catch (e) { error.value = e.message } }
...
function savePermission() { permissionOpen.value = false; notice.value = '权限标签已保存，将在下一次索引任务中生效。'; setTimeout(() => { notice.value = '' }, 2800) }
...
<template>
  <section class="knowledge-page">
    ...
    <article class="card settings-panel ledger-panel"><div class="panel-heading"><div><b>知识单元台账</b><small>编号、格式、分类、权限与启用状态一览</small></div><button class="secondary" @click="permissionOpen=true">批量设置权限</button></div><div class="table-wrap"><table><thead><tr><th>编号</th><th>标题</th><th>格式</th><th>所属分类</th><th>权限标签</th><th>更新时间</th><th>启用状态</th><th></th></tr></thead><tbody><tr v-for="doc in ledger" :key="doc.id"><td><span class="mono">{{doc.id}}</span></td><td><div class="doc-title"><span class="doc-mark">{{doc.format.slice(0,1)}}</span><b>{{doc.title}}</b></div></td><td>{{doc.format}}</td><td><span class="tag neutral">{{doc.category}}</span></td><td><span class="tag acl">{{doc.acl}}</span></td><td>{{doc.updated}}</td><td><span class="state" :class="doc.enabled?'on':'off'"><i></i>{{doc.enabled?'已启用':'已停用'}}</span></td><td><button class="text-button" @click="permissionOpen=true">权限</button></td></tr></tbody></table></div></article>
