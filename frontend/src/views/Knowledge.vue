...
let timer
const stageLabel = { queued: '排队中', parsing: '解析中', chunking: '切片中', embedding: '向量化中', indexing: '写入索引', succeeded: '已完成', failed: '失败' }
const demoDocs = [
  { id: 'KU-0001', title: '员工差旅管理制度', format: 'PDF', category: '制度规范', acl: '研发部及下级', updated: '今天 09:42', enabled: true },
  { id: 'KU-0002', title: 'IT 服务台常见问题', format: 'DOCX', category: 'FAQ', acl: '全局公开', updated: '昨天 16:08', enabled: true },
  { id: 'KU-0003', title: '采购与合同审批手册', format: 'DOC', category: '流程手册', acl: '财务部 · 采购角色', updated: '2026-09-12', enabled: false },
]
const ledger = computed(() => [...docs.value, ...demoDocs])
const selectedName = computed(() => selected.value?.name || '请选择知识库')
async function load() { try { bases.value = await api('/api/knowledge-bases'); selected.value ||= bases.value[0]; if(selected.value){ const rows=await api(`/api/knowledge-bases/${selected.value.id}/documents`); docs.value=rows.map(x=>({id:x.id,title:x.title,format:'DOC',category:'未分类',acl:x.acl?.length?`${x.acl.length} 条策略`:'未配置',updated:new Date().toLocaleDateString('zh-CN'),enabled:x.enabled})) } error.value = '' } catch (e) { error.value = e.message } }
...
async function pollJobs() {
  for (const job of jobs.value.filter(x => !['succeeded', 'failed'].includes(x.stage))) {
    try { const latest = await api(`/api/knowledge-bases/imports/${job.id}`); Object.assign(job, latest); job.progress = latest.total_units ? Math.min(100, Math.round(latest.completed_units / latest.total_units * 100)) : ({ queued: 8, parsing: 34, chunking: 58, embedding: 78, indexing: 92 }[latest.stage] || 12) } catch (_) {}
  }
  if (jobs.value.some(x => !['succeeded', 'failed'].includes(x.stage))) timer = setTimeout(pollJobs, 2200)
}
...
<template>
  <section class="knowledge-page">
    ...
    <article class="card settings-panel ledger-panel"><div class="panel-heading"><div><b>知识单元台账</b><small>编号、格式、分类、权限与启用状态一览</small></div><button class="secondary" @click="openPermission()">批量设置权限</button></div><div class="table-wrap"><table><thead><tr><th>编号</th><th>标题</th><th>格式</th><th>所属分类</th><th>权限标签</th><th>更新时间</th><th>启用状态</th><th></th></tr></thead><tbody><tr v-for="doc in ledger" :key="doc.id"><td><span class="mono">{{doc.id}}</span></td><td><div class="doc-title"><span class="doc-mark">{{doc.format.slice(0,1)}}</span><b>{{doc.title}}</b></div></td><td>{{doc.format}}</td><td><span class="tag neutral">{{doc.category}}</span></td><td><span class="tag acl">{{doc.acl}}</span></td><td>{{doc.updated}}</td><td><span class="state" :class="doc.enabled?'on':'off'"><i></i>{{doc.enabled?'已启用':'已停用'}}</span></td><td><button class="text-button" @click="openPermission(doc)">权限</button></td></tr></tbody></table></div></article>
    <article v-if="jobs.length" class="card import-jobs"><div class="panel-heading"><div><b>导入任务进度</b><small>MinerU 解析 → LangGraph 编排 → Embedding → Milvus</small></div><span class="tag neutral">{{jobs.filter(j => j.stage==='succeeded').length}} / {{jobs.length}} 已完成</span></div><div v-for="job in jobs" :key="job.id" class="job-row"><div class="job-icon">↥</div><div class="job-main"><div><b>{{job.title}}</b><span class="stage-label">{{stageLabel[job.stage] || job.stage}}</span></div><div class="progress"><i :style="{width: `${job.progress || 0}%`}"></i></div></div><strong>{{job.progress || 0}}%</strong></div></article>
