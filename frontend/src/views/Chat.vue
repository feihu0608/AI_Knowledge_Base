...
const question = ref(''), messages = ref([]), busy = ref(false), conversation = ref(null), activeHistory = ref(null)
...
<template>
  <section class="chat-page"><div class="chat-layout">
    <aside class="history-panel card"><div class="history-head"><div><b>历史会话</b><small>仅显示当前用户</small></div><button class="icon-button" title="新建会话" @click="newChat">＋</button></div><button class="new-chat" @click="newChat">＋ 开始新对话</button><div class="history-list"><button v-for="item in history" :key="item.id" class="history-item" :class="{active:activeHistory===item.id}" @click="activeHistory=item.id"><span class="history-dot"></span><div><b>{{item.title}}</b><small>{{item.time}}</small></div></button></div><div class="history-foot"><span class="status-dot"></span>三路检索已启用</div></aside>
    <div class="chat-main"><div class="chat-heading"><div><div class="eyebrow">THREE-WAY ANSWERING</div><h1>AI 智能问答工作台</h1><p>本地 Milvus 知识库 + 百炼 MCP + 模型一般知识，合并生成可追溯答案。</p></div><span class="online-pill"><i></i>服务在线</span></div>
      ...
      <form class="composer" @submit.prevent="ask"><textarea v-model="question" :disabled="busy" placeholder="输入企业知识问题…（Ctrl + Enter 发送）" @keydown.ctrl.enter.prevent="ask"></textarea><button :disabled="busy || !question.trim()">{{busy?'生成中…':'发送 ↗'}}</button></form><small class="composer-tip">回答由已授权知识生成，请在使用前核对引用来源。</small>
    </div>
  </div></section>
</template>
